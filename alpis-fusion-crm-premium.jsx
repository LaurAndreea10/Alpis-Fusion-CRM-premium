...
const createDemoState = () => {
  ...
  const clients = names.map((name, i) => ({
    ...
    phone: `072200000${i + 1}`, tags: tagSets[i], notes: notesSets[i],
  ...
  return {
    ...
    tasks: [
      { id: uid(), title: "Sună clienții inactivi", lane: "todo" },
      { id: uid(), title: "Campania de Paște", lane: "doing" },
      { id: uid(), title: "Oferta lunii publicată", lane: "done" },
    ],
    activityLog: [
...
.nav-btn { width:100%; display:flex; align-items:center; gap:9px; padding:8px 10px; border-radius:9px; font-size:12.5px; font-weight:500; color:var(--text-secondary); position:relative; transition:var(--transition); text-align:left; }
.nav-btn:hover { background:rgba(255,255,255,.04); color:var(--text); }
.nav-btn.active { background:var(--brand-dim); color:var(--brand-light); }
...
/* Buttons */
.btn { border:1px solid var(--line); background:var(--surface); color:var(--text); padding:8px 14px; border-radius:9px; font-size:12.5px; font-weight:500; transition:all .22s ease; display:inline-flex; align-items:center; gap:6px; white-space:nowrap; }
.btn:hover { background:var(--surface-2); border-color:var(--line-hover); transform:translateY(-1px); }
...
.btn.danger { color:var(--danger); border-color:rgba(255,82,102,.2); }
...
/* Items */
.item { display:flex; justify-content:space-between; align-items:center; gap:12px; padding:10px 12px; border-radius:10px; background:rgba(255,255,255,.02); border:1px solid transparent; transition:all .25s ease; }
.item:hover { border-color:var(--line); background:rgba(255,255,255,.035); }
...
/* Row actions */
.row-actions { display:flex; gap:4px; opacity:.3; transition:opacity .2s; }
tr:hover .row-actions, .item:hover .row-actions { opacity:1; }
.row-actions .btn { padding:4px 6px; border-radius:5px; font-size:11px; }
...
// ─── Empty forms ───
const emptyClient = { name: "", email: "", phone: "", tags: "", notes: "" };
...
export default function AlpisFusionCRM() {
  ...
  // ── CRUD ──
  const addClient = () => { if (!clientForm.name.trim()) return toast("Numele este obligatoriu.", "error"); if (isDuplicateEmail(clientForm.email.trim())) return toast("Email deja existent: " + clientForm.email.trim(), "error"); const c = { id: uid(), name: clientForm.name.trim(), email: clientForm.email.trim(), phone: clientForm.phone.trim(), notes: clientForm.notes.trim(), tags: parseTags(clientForm.tags), createdAt: todayISO(), lastVisit: "", conversation: [] }; setState((s) => ({ ...s, clients: [c, ...s.clients] })); closeModal2(); logAct(`Client adăugat: ${c.name}`); toast(`Client „${c.name}" adăugat.`); };
  ...
  const startEditClient = (c) => { setEditId(c.id); setClientForm({ name: c.name, email: c.email || "", phone: c.phone || "", tags: (c.tags || []).join(", "), notes: c.notes || "" }); setModal("edit-client"); };
  ...
  const toggleInvoice = (id) => { setState((s) => ({ ...s, invoices: s.invoices.map((i) => i.id === id ? { ...i, status: i.status === "paid" ? "unpaid" : "paid" } : i) })); toast("Status actualizat."); };
  const runAutomations = () => { const n = state.bookings.filter((b) => b.status === "pending").length; if (!n) return toast("Nimic de confirmat.", "error"); setState((s) => ({ ...s, bookings: s.bookings.map((b) => b.status === "pending" ? { ...b, status: "confirmed" } : b) })); toast(`${n} programări confirmate.`); };
  ...
  // Add task
  const addTask = (lane = "todo") => { if (!newTaskTitle.trim()) return; setState((s) => ({ ...s, tasks: [{ id: uid(), title: newTaskTitle.trim(), lane }, ...s.tasks] })); setNewTaskTitle(""); toast("Task adăugat."); };
            ...
            <div className="tour-card">
              ...
              {tourStep < tourSteps.length ? (
                ...
                <div style={{ textAlign: "center" }}>
                  ...
                  <p style={{ fontSize: 13, color: "var(--text-secondary)", marginBottom: 20 }}>Ai creat un flux complet: client → programare → factură → automatizare.</p>
                  <button className="btn primary" onClick={() => setPhase("app")} style={{ width: "100%", justifyContent: "center", padding: 12 }}>Explorează dashboardul →</button>
                </div>
              )}
          ...
          <main className="content">
                  ...
                  {calDays.map((day) => {
                    ...
                    return (
                      <div className="cal-day" key={day}>
                        ...
                        {bks.length ? bks.map((b) => { const c = state.clients.find((x) => x.id === b.clientId); return <div className="cal-slot" key={b.id}>{b.time} · {c?.name || "?"}</div>; }) : <p style={{ fontSize: 9, color: "var(--muted)", marginTop: 6 }}>—</p>}
                      </div>
                    );
                  })}
                </div></div>
            ...
            {view === "billing" && (
              <div className="stack-y">
                ...
                <div className="card"><div className="table-wrap"><table><thead><tr><th>ID</th><th>Client</th><th>Sumă</th><th>Status</th><th>Data</th><th></th></tr></thead><tbody>
                  {invoiceRows.map((r) => (
            ...
            {view === "automation" && (
              <div className="stack-y">
                <div className="section-head between"><div><h2 className="section-title">Automatizări</h2><p className="section-sub">Economisești 3h/săptămână</p></div><button className="btn success sm" onClick={runAutomations}>⚡ Rulează acum</button></div>
                <div className="card">
                  <span className="card-title"><span className="dot" style={{ background: "var(--accent)" }} />Flow builder — flux automat</span>
                  ...
                  <div className="flow-stage-list">
                    ...
                    <div className="flow-stage-item"><strong>CREATE</strong><span>task follow-up dacă nu răspunde</span><span className="chip yellow">fallback</span></div>
                  </div>
                </div>
                <div className="grid c2">
                  <div className="card"><span className="card-title">Reguli active</span>{state.automations.map((a) => <div className="item" key={a.id}><div className="item-info"><h4>{a.name}</h4><p>{a.type}</p></div><button className={`chip ${a.active ? "green" : "gray"}`} style={{ cursor: "pointer", border: "none", font: "inherit" }} onClick={() => toggleAutomation(a.id)}>{a.active ? "● on" : "○ off"}</button></div>)}</div>
                  <div className="card"><span className="card-title">Task-uri generate</span>{state.tasks.slice(0, 5).map((t) => <div className="item" key={t.id}><div className="item-info"><h4>{t.title}</h4><p>{t.lane}</p></div></div>)}</div>
                </div>
              </div>
            )}
        ...
        {detailClient && detailData && (
          <>
            ...
            <div className="detail-panel">
              ...
              <div className="item"><div className="item-info"><h4>Scor</h4><p>{detailData.score}/100</p></div><span className={`chip ${detailData.score > 70 ? "green" : detailData.score > 40 ? "yellow" : "red"}`}>{detailData.score > 70 ? "Hot" : detailData.score > 40 ? "Warm" : "Cold"}</span></div>
